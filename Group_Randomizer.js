...
var num_people = people.length; // save length
...
while (num_people > 1) { // as long as there are still two people to be paired(since people gets spliced)
    [1,2].forEach( function(num) { // place two people [1,2] next to each other in the new object
       var index =  Math.floor(Math.random() * num_people); // choose a random index based on the number of people
	   console.log(num);
       rst[num] = people[index];
       people.splice(index, 1);
       num_people--; // decrement the number of people left from which to create a unique group
   });
